...
if nml['psop_nml'].has_key('delz_const'):
    ...
    delz_const = 0.001
if nml['psop_nml'].has_key('lapsemin'):
    lapsemin = nml['psop_nml']['lapsemin']
else:
    lapsemin = 0.001 # min allowed lapse rate
datapath = os.path.join(nml['psop_nml']['datapath'],member)

def preduce(ps,tpress,tv,lapse,zmodel,zob):
...
# lapse - temp lapse rate (positive, K/km)
...
# Constants: rd, grav
   ...
   preduce = ps*((t0 + lapse*(zob-zmodel))/t0)**(1./alpha) # eqn 1 from B&M
   return preduce
...
lapsemodel = np.empty((ntimes,6,res,res),np.float64)
...
#if comm.rank == 0:
#    print 'min/max/mean lapse rate at level %s-%s %s' % (nlevt1,nlevt2,member), lapsemodel.min(), lapsemodel.max(), lapsemodel.mean()
#    import matplotlib.pyplot as plt
#    for ntile in range(6):
#        ntilep1 = ntile + 1
#        plt.figure()
#        clevs = np.arange(-0.01,0.0101,0.001)
#        cs = plt.contourf(lapsemodel[1,ntile,:,:],clevs,cmap=plt.cm.bwr,extend='both')
#        plt.title('lapserate tile %s' % ntilep1)
#        plt.colorbar()
#    plt.show()
#raise SystemExit
lapsemodel = np.clip(lapsemodel, lapsemin, 1.e30)
lapsemodel = lapsemodel.reshape((ntimes,6*res*res))
if comm.rank == 0:
    ...
    print 'min/max/mean t at level %s %s' % (nlevt1,member), tmodel.min(), tmodel.max(), tmodel.mean()
    print 'min/max/mean p at level %s %s' % (nlevt1,member), pmodel.min(), pmodel.max(), pmodel.mean()
    print 'min/max/mean lapse rate at level %s-%s %s' % (nlevt1,nlevt2,member), lapsemodel.min(), lapsemodel.max(), lapsemodel.mean()
...
lapsemodel_interp = np.empty((ntimes,nobs),np.float64)
for ntime in range(ntimes):
    ...
    lapsemodel_interp[ntime] = tri.interp_linear(olons,olats,lapsemodel[ntime])
...
anal_oblapse = np.empty(nobs, np.float64)
for nob in range(nobs):
    ...
    anal_oblapse[nob] = (1.-delt[nob])*lapsemodel_interp[idtob[nob],nob] + delt[nob]*lapsemodel_interp[idtobp[nob],nob]

# adjust interpolated model forecast pressure to station height
anal_ob = preduce(anal_ob, anal_obp, anal_obt, anal_oblapse, zobs, zsmodel_interp)
